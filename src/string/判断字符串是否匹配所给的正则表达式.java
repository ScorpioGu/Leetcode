            ...
            for (int j = 1; j <= p.length(); j++) {
                ...
                if (p.charAt(j - 1) == '*') {
                    ...
                    if (p.charAt(j - 2) != s.charAt(i - 1) && p.charAt(j - 2) != '.' ) {
                        ...
                        //为0是，等于把最后两个字符珊 对应于valid[i][j-2],
                        //为1，则是去掉最后一个字符，对应于valid[i][j-1]
                        //为多个，对应于valid[i-1][j]，s上新加的字符被*抵消
                        valid[i][j] = (valid[i][j - 2] || valid[i][j - 1] || valid[i - 1][j]);
                    }
